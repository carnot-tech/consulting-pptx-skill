...
作り方は1本です。HTML（16:9・1 section = 1スライド）で組み、Chrome で PDF にして納品します。旧 SlideSpec パイプライン（JSON → 編集可能PPTX）は使用頻度が低かったため廃止し（git タグ `pipeline-archived` で辿れる）、その36型はHTMLパーツとして追加パーツ集に移しました。
