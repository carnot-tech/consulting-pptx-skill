...
export function html(ctx, item, n) {
  ...
  const sizes = items.map((it) => Math.max(0, Number(it.size) || 0));
  const sizeMax = Math.max(...sizes, 1e-9);
  const fills = ["var(--navy)", "var(--blue)", "var(--cyan)", "var(--muted)"];
  const subs = ["var(--cyan)", "var(--soft-blue)", "var(--ink)", "#fff"];
  const T = H.svgText;
  let svg = `<svg class="pfig" viewBox="0 0 900 300" preserveAspectRatio="xMidYMid meet">`;
  items.forEach((it, i) => {
    const cx = cnt === 2 ? (i === 0 ? 260 : 640) : (900 * (i + 0.5)) / cnt;
    const r = sizes[i] > 0 ? 120 * Math.sqrt(sizes[i] / sizeMax) : 30;
    const k = r / 120;
    const vSize = Math.max(18, Math.round(52 * k)), lSize = Math.max(11, Math.round(15 * Math.max(k, 0.8)));
    svg += T(e, cx, 20, it.heading || "", { size: 16, bold: true, anchor: "middle" });
    svg += `<circle cx="${cx}" cy="150" r="${r.toFixed(1)}" style="fill:${fills[i % fills.length]}"/>`;
    svg += T(e, cx, 150, it.value ?? "", { size: vSize, fill: "#fff", anchor: "middle", bold: true, baseline: "central", cls: "serif" });
    if (it.label) svg += T(e, cx, 150 + vSize * 0.55 + lSize, it.label, { size: lSize, fill: subs[i % subs.length], anchor: "middle" });
  });
  svg += "</svg>";
