...
export function html(ctx, item, n) {
  ...
  const left = p.left || {}, right = p.right || {};
  const trs = (left.rows || [])
    .map((r, i) => `<tr><td class="ax"${i === 0 ? ` style="width:${H.px(44)}px"` : ""}>${H.cell(e, r[0])}</td><td class="b" style="${i === 0 ? `width:${H.px(34)}px;` : ""}font-size:23px">${H.cell(e, r[1])}</td><td>${H.cell(e, r[2])}</td></tr>`)
    .join("");
  return H.wrap(ctx, item, n,
    `<div class="two"><div>${H.colh(e, left.header)}<table>${trs}</table></div>${H.tri()}<div>${H.colh(e, right.header)}${H.ul(e, right.bullets)}</div></div>`);
}
