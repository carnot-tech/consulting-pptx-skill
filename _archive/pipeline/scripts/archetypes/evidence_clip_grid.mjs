...
export function html(ctx, item, n) {
  ...
  const clips = Array.isArray(p.clips) ? p.clips : [];
  const g = clips.length <= 4 ? 2 : 3;
  const cs = clips
    .map((c) => `<div class="clip">${c.tag ? `<span class="cg">${e(c.tag)}</span>` : ""}${c.date ? `<span class="cd">${e(c.date)}</span>` : ""}${c.headline ? `<span class="ch">${e(c.headline)}</span>` : ""}${c.text ? `<span class="cx">${e(c.text)}</span>` : ""}</div>`)
    .join("");
  return H.wrap(ctx, item, n, `<div class="grid g${g}">${cs}</div>`);
}
