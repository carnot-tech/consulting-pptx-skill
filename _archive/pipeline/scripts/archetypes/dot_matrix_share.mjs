...
export function html(ctx, item, n) {
  ...
  const dotcols = cols
    .map((c) => {
      const pct = Number.isFinite(Number(c.pct)) ? Number(c.pct) : parseFloat(String(c.value || "").replace(/[^\d.]/g, "")) || 0;
      const on = Math.max(0, Math.min(100, Math.round(pct)));
      const dots = Array.from({ length: 100 }, (_, k) => `<i${k < on ? ' class="on"' : ""}></i>`).join("");
      return `<div class="dotcol"><div class="dv">${e(c.value ?? "")}</div><div class="dots">${dots}</div><div class="dl">${e(c.label ?? "")}</div></div>`;
    })
    .join("");
  const g = Math.min(5, Math.max(2, cols.length));
  return H.wrap(ctx, item, n, `${p.header || p.unit ? H.axh(e, p.header, p.unit) : ""}<div class="grid g${g}" style="flex:1;align-items:center">${dotcols}</div>`);
}
