...
export function html(ctx, item, n) {
  const e = ctx.esc;
  const rows = ((item.parts && item.parts.items) || [])
    .map((it) => `<div class="ai${it.current ? " on" : ""}"><span class="an">${e(it.n)}</span><span class="at">${e(it.text)}</span><span class="ap">${e(it.page)}</span></div>`)
    .join("");
  return H.wrap(ctx, item, n, `<div class="agd">${rows}</div>`);
}
