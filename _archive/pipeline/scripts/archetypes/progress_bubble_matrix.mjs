...
export function html(ctx, item, n) {
  ...
  const nCols = Math.max(1, heads.length, ...rows.map((r) => (r.values || []).length));
  const nRows = Math.max(1, rows.length);
  const rowPitch = 80, labelW = 165;
  const H_ = 40 + rowPitch * (nRows - 1) + 40;
  const colPitch = (900 - labelW - 35) / nCols;
  const cx = (ci) => labelW + (ci + 0.5) * colPitch;
  const cy = (ri) => 40 + ri * rowPitch;
  const rMax = Math.min(rowPitch * 0.42, colPitch * 0.36);
  const vMax = Math.max(1, ...rows.flatMap((r) => (r.values || []).map((v) => Number(v) || 0)));
  const T = H.svgText;
  let svg = `<svg class="pfig" viewBox="0 0 900 ${H_}" preserveAspectRatio="xMidYMid meet">`;
  svg += heads.map((h, ci) => T(e, cx(ci), 18, h, { size: 14, anchor: "middle" })).join("");
  rows.forEach((r, ri) => {
    svg += T(e, 0, cy(ri), r.axis ?? "", { size: 15, bold: true, baseline: "central" });
    (r.values || []).forEach((raw, ci) => {
      const v = Number(raw) || 0;
      if (v <= 0) return;
      const rad = (rMax * Math.sqrt(v / vMax)).toFixed(1);
      svg += `<circle cx="${cx(ci)}" cy="${cy(ri)}" r="${rad}" style="fill:var(--navy)"/>` + T(e, cx(ci), cy(ri), String(raw), { size: 13, fill: "#fff", anchor: "middle", bold: true, baseline: "central" });
    });
  });
  svg += "</svg>";
  const legend = p.legend ? `<div class="legend"><span>${e(p.legend)}</span></div>` : "";
  return H.wrap(ctx, item, n, `${H.axh(e, p.header, p.unit)}${svg}${legend}`);
}
