...
export function html(ctx, item, n) {
  ...
  const colHeaders = p.colHeaders || ["指標カテゴリー", "前月比", "前年比"];
  const legendItems = p.legend || [{ v: 4, label: "大きく改善" }, { v: 3, label: "改善" }, { v: 0, label: "横ばい" }, { v: 2, label: "悪化" }];
  const lvl = (v) => `v${Math.min(4, Math.max(0, Number(v) || 0))}`;
  const legend = H.legend(e, legendItems.map((it) => ({ label: it.label, cls: lvl(it.v) })));
  const ths = colHeaders.map((h, ci) => `<th${ci === 0 ? ' class="ax"' : ""} style="width:${H.px(ci === 0 ? 38 : 26)}px">${e(h)}</th>`).join("");
  const nCmp = Math.max(1, colHeaders.length - 1);
  const trs = (p.rows || [])
    .map((r) => `<tr><td class="ax">${e(r.label ?? "")}</td>${Array.from({ length: nCmp }, (_, ci) => `<td><span class="cell ${lvl((r.values || [])[ci])}"></span></td>`).join("")}</tr>`)
    .join("");
  return H.wrap(ctx, item, n,
    `${legend}<div class="two" style="grid-template-columns:1.25fr auto 1fr"><div>${H.axh(e, p.axisTitle, p.unit)}<table class="hm"><tr>${ths}</tr>${trs}</table></div>${H.tri()}` +
    `<div>${H.colh(e, p.commentTitle ?? "だから、次に見るべき点")}${H.ul(e, p.comments)}</div></div>`);
}
