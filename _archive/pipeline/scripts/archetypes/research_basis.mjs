...
export function html(ctx, item, n) {
  ...
  const trs = (p.targets || [])
    .map((t, i) => `<tr><td class="b" style="${i === 0 ? `width:${H.px(28)}px;` : ""}font-size:28px">${e(t.n ?? "")}</td><td>${e(t.text ?? "")}</td></tr>`)
    .join("");
  return H.wrap(ctx, item, n,
    `<div class="two" style="grid-template-columns:1fr auto 1.35fr"><div>${H.colh(e, p.methodTitle ?? "調べ方")}${H.ul(e, p.methods)}</div>${H.tri()}` +
    `<div>${H.colh(e, p.targetTitle ?? "調べた対象")}<table>${trs}</table></div></div>`);
}
