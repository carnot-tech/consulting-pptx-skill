...
export function html(ctx, item, n) {
  const e = ctx.esc;
  const cvs = ((item.parts && item.parts.steps) || [])
    .map((s) => `<div class="cv">${s.n != null ? `<span class="n">${e(s.n)}</span>` : ""}<span class="t">${e(s.title || "")}</span>${s.text ? `<span class="x">${e(s.text)}</span>` : ""}</div>`)
    .join("");
  return H.wrap(ctx, item, n, `<div class="chev">${cvs}</div>`, { center: true });
}
