...
export function html(ctx, item, n) {
  ...
  const cats = (p.categories || []).filter((c) => c && c.label != null);
  const upLabel = p.upLabel || "増える分", downLabel = p.downLabel || "減る分";
  const ups = cats.map((c) => Math.abs(Number(c.up) || 0)), downs = cats.map((c) => Math.abs(Number(c.down) || 0));
  const upTotal = p.upTotal ?? ups.reduce((a, b) => a + b, 0), downTotal = p.downTotal ?? downs.reduce((a, b) => a + b, 0);
  const maxAbs = Math.max(...ups, ...downs, 1);
  const lim = Math.ceil((maxAbs * 1.15) / 10) * 10;
  const scale = 130 / lim;
  const T = H.svgText;
  let svg = `<svg class="pfig" viewBox="0 0 900 300" preserveAspectRatio="xMidYMid meet">`;
  svg += T(e, 20, 90, `＋${H.fmt(upTotal)}`, { size: 40, fill: "var(--blue)", bold: true, cls: "serif" }) + T(e, 20, 112, upLabel, { size: 14 });
  svg += T(e, 20, 230, `−${H.fmt(downTotal)}`, { size: 40, bold: true, cls: "serif" }) + T(e, 20, 252, downLabel, { size: 14 });
  const pitch = 700 / Math.max(1, cats.length), bw = Math.min(70, pitch * 0.58);
  cats.forEach((c, i) => {
    const cx = 180 + pitch * (i + 0.5);
    const hu = (ups[i] * scale).toFixed(1), hd = (downs[i] * scale).toFixed(1);
    svg += `<rect x="${(cx - bw / 2).toFixed(1)}" y="${(150 - hu).toFixed(1)}" width="${bw.toFixed(1)}" height="${hu}" style="fill:var(--blue)"/>`;
    svg += `<rect x="${(cx - bw / 2).toFixed(1)}" y="150" width="${bw.toFixed(1)}" height="${hd}" style="fill:var(--navy)"/>`;
    if (p.showValues) {
      if (ups[i]) svg += T(e, cx, 150 - hu + 16, H.fmt(ups[i]), { size: 12, fill: "#fff", anchor: "middle" });
      if (downs[i]) svg += T(e, cx, 150 + hd - 6, H.fmt(downs[i]), { size: 12, fill: "#fff", anchor: "middle" });
    }
    svg += T(e, cx, 270, String(c.label), { size: 14, anchor: "middle" });
  });
  svg += `<line x1="180" y1="150" x2="880" y2="150" style="stroke:var(--ink)" stroke-width="2"/></svg>`;
  const legend = H.legend(e, [{ label: upLabel, color: "var(--blue)" }, { label: downLabel, color: "var(--navy)" }]);
