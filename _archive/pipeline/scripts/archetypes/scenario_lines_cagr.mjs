...
export function html(ctx, item, n) {
  ...
  const cats = (p.categories || []).map(String);
  const series = (p.series || []).filter((sr) => sr && Array.isArray(sr.values)).slice(0, 4);
  const colors = ["var(--navy)", "var(--blue)", "var(--pgrey)", "var(--muted)"];
  const chipText = ["#fff", "#fff", "var(--ink)", "#fff"];
  const cnt = Math.max(cats.length, 2);
  const X = (i) => 60 + (i * 600) / (cnt - 1);
  const all = series.flatMap((sr) => sr.values.map((v) => Number(v) || 0));
  const vMax = Math.max(...all, 1);
  const yMin = Math.min(Number(p.yMin) || 0, Math.min(...all));
  const lim = Math.ceil((yMin + (vMax - yMin) * 1.08) / 10) * 10;
  const Y = (v) => 270 - (250 * ((Number(v) || 0) - yMin)) / (lim - yMin || 1);
  const T = H.svgText;
  let svg = `<svg class="pfig" viewBox="0 0 900 300" preserveAspectRatio="xMidYMid meet">`;
  svg += `<line x1="60" y1="270" x2="760" y2="270" style="stroke:var(--ink)" stroke-width="2"/>`;
  series.forEach((sr, i) => {
    const pts = cats.map((_, k) => `${X(k).toFixed(1)},${Y(sr.values[k]).toFixed(1)}`).join(" ");
    svg += `<polyline points="${pts}" fill="none" style="stroke:${colors[i % colors.length]}" stroke-width="3"/>`;
  });
  const endYs = series.map((sr) => Y(sr.values[cats.length - 1]));
  if (p.endLabels !== false) {
    const ys = H.spread(endYs, 22, 20, 270);
    series.forEach((sr, i) => { if (sr.values[cats.length - 1] != null) svg += T(e, 670, ys[i] + 5, H.fmt(sr.values[cats.length - 1]), { size: 16, bold: true }); });
  }
  const showAll = cats.length <= 8;
  cats.forEach((c, k) => { if (showAll || k === 0 || k === cats.length - 1) svg += T(e, X(k), 292, c, { size: 14, anchor: "middle" }); });
  const chips = series.filter((sr) => sr.chip != null && sr.chip !== "");
  if (chips.length) {
    svg += T(e, 770, 20, p.chipTitle || "年平均成長率", { size: 13, bold: true });
    const ys = H.spread(endYs, 34, 55, 257);
    series.forEach((sr, i) => {
      if (sr.chip == null || sr.chip === "") return;
      const y = ys[i] - 13;
      svg += `<rect x="770" y="${y.toFixed(1)}" width="100" height="26" style="fill:${colors[i % colors.length]}"/>` + T(e, 820, ys[i], String(sr.chip), { size: 15, fill: chipText[i % chipText.length], anchor: "middle", bold: true, baseline: "central" });
    });
  }
  svg += "</svg>";
  const legend = H.legend(e, series.map((sr, i) => ({ label: String(sr.name || ""), color: colors[i % colors.length] })));
  return H.wrap(ctx, item, n, `${H.axh(e, p.axisTitle, p.unit)}${svg}${legend}`);
}
