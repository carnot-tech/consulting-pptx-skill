...
export function html(ctx, item, n) {
  ...
  const rows = p.rows || [];
  const col = (head, list) => {
    const rs = list.map((r) => `<div class="r"><div class="l">${e(r.label ?? "")}</div><div><span>${e(r.text ?? "")}</span></div></div>`).join("");
    return `<div>${H.axh(e, head)}<div class="rows">${rs}</div></div>`;
  };
  const issues = rows.map((r) => r.issue || {}).filter((r) => r.label || r.text);
  const actions = rows.map((r) => r.action || {}).filter((r) => r.label || r.text);
  return H.wrap(ctx, item, n, `<div class="grid g2">${col(headers.issue, issues)}${col(headers.action, actions)}</div>`);
}
