...
export function html(ctx, item, n) {
  ...
  const headers = p.headers || ["評価軸", "ラベル 1案", "ラベル 2案", "判断の理由"];
  const legendItems = p.legend || [{ q: 4, label: "満たす" }, { q: 2, label: "一部満たす" }, { q: 0, label: "満たさない" }];
  const legend = H.legend(e, legendItems.map((it) => ({ label: it.label, html: H.hb(it.q) })));
  const nBall = Math.max(1, headers.length - 2);
  const ths = headers
    .map((h, ci) => (ci === 0 ? `<th class="ax" style="width:${H.px(50)}px">${e(h)}</th>` : ci <= nBall ? `<th class="hbc" style="width:${H.px(34)}px">${e(h)}</th>` : `<th>${e(h)}</th>`))
    .join("");
  const trs = (p.rows || [])
    .map((r) => {
      const balls = Array.from({ length: nBall }, (_, ci) => `<td class="hbc">${H.hb((r.values || [])[ci])}</td>`).join("");
      const body = Array.isArray(r.bullets) && r.bullets.length ? H.ul(e, r.bullets, "cb") : Array.isArray(r.text) ? H.ul(e, r.text, "cb") : e(r.text ?? "");
      return `<tr><td class="ax">${e(r.axis ?? "")}</td>${balls}<td>${body}</td></tr>`;
    })
    .join("");
  return H.wrap(ctx, item, n, `${legend}<table><tr>${ths}</tr>${trs}</table>`);
}
