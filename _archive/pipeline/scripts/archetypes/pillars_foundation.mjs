...
export function html(ctx, item, n) {
  ...
  const pillars = Array.isArray(p.pillars) ? p.pillars : [];
  const bases = Array.isArray(p.bases) ? p.bases : p.base ? [p.base] : [];
  const g = Math.min(5, Math.max(2, pillars.length));
  const ps = pillars.map((pl) => `<div class="p"><span class="pt">${e(pl.title || "")}</span>${pl.text ? `<span class="px">${e(pl.text)}</span>` : ""}</div>`).join("");
  const bs = bases.map((b) => `<div class="base">${b.lead ? `<b>${e(b.lead)}</b>` : ""}<span>${e(b.text || "")}</span></div>`).join("");
  return H.wrap(ctx, item, n, `<div class="pil grid g${g} fix" style="height:${H.px(62)}px">${ps}</div>${bs}`, { center: true });
}
