...
export function html(ctx, item, n) {
  ...
  const col = (items) => `<div class="ol">${(Array.isArray(items) ? items : []).map((t) => `<div class="oi">${e(t)}</div>`).join("")}</div>`;
  const center = (Array.isArray(p.center) ? p.center : []).map((b) => `<div class="ob${b.alt ? " alt" : ""}">${H.nl2br(e, b.text)}</div>`).join("");
  return H.wrap(ctx, item, n, `<div class="opp">${col(p.left)}<div class="oc">${center}</div>${col(p.right)}</div>`);
}
