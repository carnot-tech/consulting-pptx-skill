...
export function html(ctx, item, n) {
  ...
  const panel = p.panel || {}, right = p.right || {};
  const trs = (right.rows || [])
    .map((r, i) => {
      const c = Array.isArray(r) ? r : [r.axis, r.value, r.text];
      return `<tr><td class="ax"${i === 0 ? ` style="width:${H.px(40)}px"` : ""}>${H.cell(e, c[0])}</td><td class="b" style="${i === 0 ? `width:${H.px(26)}px;` : ""}font-size:23px">${H.cell(e, c[1])}</td><td>${H.cell(e, c[2])}</td></tr>`;
    })
    .join("");
  return H.wrap(ctx, item, n,
    `<div class="side"><div class="sp"><div class="cn">${e(panel.n ?? "")}</div><div class="ct">${e(panel.label ?? "")}</div><div class="cm">${e(panel.claim ?? "")}</div></div>` +
    `<div class="sb">${H.axh(e, right.axisTitle, right.unit)}<table>${trs}</table></div></div>`);
}
