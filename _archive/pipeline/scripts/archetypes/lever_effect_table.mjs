...
export function html(ctx, item, n) {
  ...
  const headers = p.headers || ["打ち手", "効果の幅（単位）", "前提・制約"];
  const lg = p.legend || {};
  const legend = H.legend(e, [{ label: lg.up || "増やす方向", cls: "li-b" }, { label: lg.dn || "減らす方向", color: "var(--pgrey)" }]);
  const ths = headers.map((h, ci) => `<th${ci === 0 ? ' class="ax"' : ""}${ci < 2 ? ` style="width:${H.px(56)}px"` : ""}>${e(h)}</th>`).join("");
  const trs = (p.rows || [])
    .map((r) => {
      const ef = r.effect || {};
      const pct = Math.round(Math.min(1, Math.max(0.08, Number(ef.pct ?? 0.5))) * 100);
      const dir = (ef.dir || "up") !== "dn" ? "up" : "dn";
      const body = Array.isArray(r.bullets) && r.bullets.length ? H.ul(e, r.bullets, "cb") : Array.isArray(r.text) ? H.ul(e, r.text, "cb") : e(r.text ?? "");
      return `<tr><td class="ax">${e(r.axis ?? "")}</td><td><span class="ar ${dir}" style="width:${pct}%">${e(ef.label ?? "")}</span></td><td>${body}</td></tr>`;
    })
    .join("");
  return H.wrap(ctx, item, n, `${legend}<table class="lev"><tr>${ths}</tr>${trs}</table>`);
}
