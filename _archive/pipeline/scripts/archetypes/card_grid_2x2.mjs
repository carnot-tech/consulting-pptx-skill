...
export function html(ctx, item, n) {
  ...
  const cards = ((item.parts && item.parts.cards) || []).slice(0, 4)
    .map((c) => {
      const body = Array.isArray(c.bullets) && c.bullets.length ? H.ul(e, c.bullets) : e(c.body || "");
      return `<div class="card"><div class="hd">${c.n != null ? `<span class="no">${e(c.n)}</span>` : ""}<span class="h">${e(c.title || "")}</span></div><div class="bd">${body}</div></div>`;
    })
    .join("");
  return H.wrap(ctx, item, n, `<div class="grid g2">${cards}</div>`);
}
